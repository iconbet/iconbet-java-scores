...
public class Dividend extends Utils {
	...
	public boolean distribute() {
		...
		if (this._dividends_received.getOrDefault(ZERO).equals(ONE)) {
			...
			if (this._update_stake_balances()) {
				...
				Context.call(scoreAddresses._token_score.get(), "switch_stake_update_db");
			...
			boolean treasuryStatus = Context.call(Boolean.class, scoreAddresses._game_score.get(), "get_treasury_status");
			if (treasuryStatus) {
			...
			BigInteger batchSize = Context.call(BigInteger.class, scoreAddresses._game_score.get(), "get_batch_size", BigInteger.valueOf(this._stake_holders.size()));
			...
			Context.call(scoreAddresses._token_score.get(), "clear_yesterdays_stake_changes");
			return true;
	...
	private boolean _update_stake_balances() {
		ScoreAddresses scoreAddresses = new ScoreAddresses();
		Map<String, BigInteger> stake_balances = (Map<String, BigInteger>) Context.call(scoreAddresses._token_score.get(), "get_stake_updates");
	...
	private void _set_total_tap() {
		...
		for (int i = 0; i < this._blacklist_address.size(); i++) {
			String address = this._blacklist_address.get(i);
			Address address_from_str = Address.fromString(address);
			BigInteger balance = (BigInteger) Context.call(scoreAddresses._token_score.get(), "balanceOf", address_from_str);
			total = total.add(balance);
		}
		BigInteger totalSupply = (BigInteger) Context.call(scoreAddresses._token_score.get(), "totalSupply");
	...
	private void _set_total_staked_tap() {
		ScoreAddresses scoreAddresses = new ScoreAddresses();
		BigInteger stakedBalance = (BigInteger) Context.call(scoreAddresses._token_score.get(), "total_staked_balance");
	...
	private void _set_games() {
		ScoreAddresses scoreAddresses = new ScoreAddresses();
		Map<String, String> gamesExcess = (Map<String, String>) Context.call(scoreAddresses._game_auth_score.get(), "get_yesterdays_games_excess");
		...
		while (it.hasNext()) {
			...
			if (gameEx.compareTo(ZERO) > 0) {
				...
				Address revshare = (Address) Context.call(scoreAddresses._game_auth_score.get(), "get_revshare_wallet_address", game_address);
	...
	public void _set_tap_of_exception_address() {
		...
		for (int idx = 0; idx < this._exception_address.size(); idx++) {
			...
			BigInteger tap_balance = Context.call(BigInteger.class, scoreAddresses._token_score.get(), "balanceOf", add);
			BigInteger staked_balance = Context.call(BigInteger.class, scoreAddresses._token_score.get(), "staked_balanceOf", add);
